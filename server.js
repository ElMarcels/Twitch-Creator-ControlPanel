...
async function refreshKickAccessToken(tokenData) {
...
async function kickAPI(channelId, endpoint, options = {}) {
  const tokenData = kickTokens.get(channelId);
  if (!tokenData) return { status: 401, data: { error: 'Kick not connected' } };

  const url = endpoint.startsWith('http') ? endpoint : `https://api.kick.com${endpoint}`;
  const method = options.method || 'GET';
  const headers = {
  ...
  try {
    ...
    if (resp.status === 401) {
      const refreshed = await refreshKickAccessToken(tokenData);
      if (refreshed) {
        kickTokens.set(channelId, refreshed);
...
app.get('/auth/kick/callback', async (req, res) => {
  ...
  try {
    ...
    if (!tokenData.access_token) {
      return res.redirect('/dashboard?kick=token_failed');
    }
    ...
    if (!user) {
      return res.redirect('/dashboard?kick=no_user');
    }

    kickTokens.set(channelId, {
      accessToken: tokenData.access_token,
      refreshToken: tokenData.refresh_token,
      expiresAt: Date.now() + (tokenData.expires_in * 1000),
      user: {
    ...
    await saveKickTokensToRedis();
    res.redirect('/dashboard?kick=connected');
  } catch (err) {
    console.error('Kick auth error:', err);
    res.redirect('/dashboard?kick=error');
  }
});
...
app.post('/api/platforms/disconnect/:platform', requireAuth, async (req, res) => {
...
app.get('/api/multistream/status', requireAuth, async (req, res) => {
  ...
  try {
    const streamResp = await twitchAPI(req, res, '/streams', {
      headers: { 'Client-ID': TWITCH_CLIENT_ID }
    });
    if (streamResp.status === 200 && streamResp.data && streamResp.data.data && streamResp.data.data.length > 0) {
      const s = streamResp.data.data[0];
      result.twitch = {
        live: true,
        title: s.title,
  ...
  if (kickTokens.has(channelId)) {
    try {
      const kickUser = kickTokens.get(channelId)?.user;
      if (kickUser && kickUser.slug) {
        const kickResp = await kickAPI(channelId, `/public/v1/channels/${kickUser.slug}`);
        if (kickResp.status === 200 && kickResp.data && kickResp.data.data) {
          const ch = kickResp.data.data;
          if (ch.stream && ch.stream.is_live) {
            result.kick = {
              live: true,
              title: ch.stream.title || ch.stream_title,
              category: ch.stream.category?.name || '',
              viewers: ch.stream.viewer_count || 0,
              startedAt: ch.stream.start_time,
              thumbnail: ch.stream.thumbnail
            };
          } else {
            result.kick = { live: false };
          }
        } else {
          result.kick = { live: false };
        }
...
app.get('/api/multistream/stream-info', requireAuth, async (req, res) => {
  const channelId = req.auth.user.id;
  const platforms = [];
    ...
    try {
      ...
      if (kickUser && kickUser.slug) {
        ...
        if (kickResp.status === 200 && kickResp.data && kickResp.data.data) {
          ...
          if (ch.stream && ch.stream.is_live) {
            kickInfo.live = true;
            kickInfo.title = ch.stream.title || ch.stream_title;
            kickInfo.category = ch.stream.category?.name || '';
            kickInfo.viewers = ch.stream.viewer_count || 0;
          }
        }
      }
    } catch {}
...
// ===== KICK STREAM CONFIG =====
app.get('/api/kick/categories', requireAuth, async (req, res) => {
  const channelId = req.auth.user.id;
  if (!kickTokens.has(channelId)) return res.status(400).json({ error: 'Kick not connected' });
  try {
    const resp = await kickAPI(channelId, '/public/v1/categories');
    res.json(resp.data);
  } catch (err) {
...
app.put('/api/kick/stream-config', requireAuth, async (req, res) => {
  const channelId = req.auth.user.id;
  if (!kickTokens.has(channelId)) return res.status(400).json({ error: 'Kick not connected' });
  ...
  try {
    const kickUser = kickTokens.get(channelId)?.user;
    if (!kickUser || !kickUser.id) return res.status(400).json({ error: 'Kick user not found' });

    const updateBody = {};
    if (title) updateBody.title = title;
    if (categoryId) updateBody.category_id = categoryId;

    const resp = await kickAPI(channelId, '/public/v1/channels', {
      method: 'PATCH',
      body: updateBody,
      headers: { 'Content-Type': 'application/json' }
    });
...
// ===== KICK CHAT =====
app.get('/api/kick/chat/messages', requireAuth, async (req, res) => {
  const channelId = req.auth.user.id;
  if (!kickTokens.has(channelId)) return res.status(400).json({ error: 'Kick not connected' });

  try {
    const kickUser = kickTokens.get(channelId)?.user;
    if (!kickUser || !kickUser.slug) return res.json({ data: { messages: [] } });
    const resp = await kickAPI(channelId, `/public/v1/channels/${kickUser.slug}`);
    if (resp.status === 200 && resp.data && resp.data.data && resp.data.data.chatroom) {
      return res.json({ data: { chatroomId: resp.data.data.chatroom.id, messages: [] } });
    }
...
app.post('/api/kick/chat/send', requireAuth, async (req, res) => {
  const channelId = req.auth.user.id;
  if (!kickTokens.has(channelId)) return res.status(400).json({ error: 'Kick not connected' });

  const { chatroomId, content } = req.body;
  if (!chatroomId || !content) return res.status(400).json({ error: 'chatroomId and content required' });

  try {
    const resp = await kickAPI(channelId, `/public/v1/chat/${chatroomId}`, {
      method: 'POST',
      body: { content }
    });
...
app.post('/api/kick/mod/ban', requireAuth, async (req, res) => {
  const channelId = req.auth.user.id;
  if (!kickTokens.has(channelId)) return res.status(400).json({ error: 'Kick not connected' });

  const { broadcasterUserId, userId, duration, reason } = req.body;
  if (!broadcasterUserId || !userId) return res.status(400).json({ error: 'broadcasterUserId and userId required' });
